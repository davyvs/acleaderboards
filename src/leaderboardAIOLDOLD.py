...
def score_find(selected_log: str, previous_log: str, combined_server_path_rel: str):
    ...
    for index_log_line,log_line in enumerate(selected_log_lines):
        ...
        or (shmoovin_match := re.findall(".* \[INF\] CHAT: (.*) \(\d*\): just scored a (\d*)", log_line))):
            ...
            # Try to extract time and combo from the same line
            run_time = None
            combo_peak = None
            # Match pattern: "just scored a 12345 | Time: 00:00.00 | Combo: 123.4x"
            if time_combo_match := re.findall(r"just scored a \d* \| Time: ([\d:\.]+) \| Combo: ([\d\.]+)x", log_line):
                run_time = str(time_combo_match[0][0])
                combo_peak = str(time_combo_match[0][1])
                logging.debug(f"found time and combo in same line: Time={run_time}, Combo={combo_peak}")
            
            score_find_additional(name, score, leaderboard_file_name, index_log_line, selected_log, previous_log, combined_server_path_rel, run_time, combo_peak)
...
def score_find_additional(name: str,score: float, leaderboard_file_name: str,index_log_line: int, selected_log: str, previous_log: str, combined_server_path_rel: str, run_time: str = None, combo_peak: str = None):
    ...
    if name_allowed:
        ...
        write_score(name, score, car, input_method, leaderboard_file_name, combined_server_path_rel, run_time, combo_peak)
...
def write_score(name, score, car, input_method, leaderboard_file_name, combined_server_path_rel, run_time=None, combo_peak=None):
    ...
    with open(f"{os.path.join(combined_server_path_rel,leaderboard_file_name)}", encoding='utf-8', errors='ignore', mode="r+") as score_file:
        ...
        for score_file_line in score_file_lines:
            ...
            if name in score_file_line and car in score_file_line:
                    ...
                    if leaderboard_file_name == "leaderboard.txt" and float(score) > float(old_score):
                        # Include run_time and combo_peak if available
                        if run_time and combo_peak:
        ...
        if was_found == False:
            # Include run_time and combo_peak if available
            if leaderboard_file_name == "leaderboard.txt" and run_time and combo_peak:
                entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak}\n"
            else:
                entry = f"{car},{name},{score},{input_method}\n"
            score_file_lines_new.append(entry)
    ...
    with open(os.path.join(combined_server_path_rel,score_type), 'r', encoding='utf-8', errors='ignore') as score_file:
        ...
        for line in score_file:
            ...
            if name_allowed_to_sort:
                scores.append([car, name, score, input_method, run_time, combo_peak])
    ...
    for i,score in enumerate(scores):
        ...
        for classcore in score:
            ...
            if scorecounter <= scorelength:
                ...
                # Get run_time and combo_peak if available (indices 4 and 5)
                ...
                # Format entry with optional time and combo
                extra_info = ""
                if run_time and combo_peak:
