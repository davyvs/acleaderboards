...
def score_find(selected_log: str, previous_log: str, combined_server_path_rel: str):
    ...
    for index_log_line,log_line in enumerate(selected_log_lines):
        ...
        or (shmoovin_match := re.findall(".* \[INF\] CHAT: (.*) \(\d*\): just scored a (\d*)", log_line))):
            ...
            # Try to extract time, combo, and average speed from the same line
            run_time = None
            combo_peak = None
            avg_speed = None
            # Match pattern: "just scored a 12345 | Time: 00:00.00 | Combo: 123.4x | Avg: 150 km/h"
            ...
            score_find_additional(name, score, leaderboard_file_name, index_log_line, selected_log, previous_log, combined_server_path_rel, run_time, combo_peak, avg_speed)
...
def score_find_additional(name: str,score: float, leaderboard_file_name: str,index_log_line: int, selected_log: str, previous_log: str, combined_server_path_rel: str, run_time: str = None, combo_peak: str = None, avg_speed: str = None):
    ...
    if name_allowed:
        ...
        write_score(name, score, car, input_method, leaderboard_file_name, combined_server_path_rel, run_time, combo_peak, avg_speed)
...
def write_score(name, score, car, input_method, leaderboard_file_name, combined_server_path_rel, run_time=None, combo_peak=None, avg_speed=None):
    ...
    with open(f"{os.path.join(combined_server_path_rel,leaderboard_file_name)}", encoding='utf-8', errors='ignore', mode="r+") as score_file:
        ...
        for score_file_line in score_file_lines:
            ...
            if name in score_file_line and car in score_file_line:
                    ...
                    if leaderboard_file_name == "leaderboard.txt" and float(score) > float(old_score):
                        # Include run_time, combo_peak, and avg_speed if available
                        if run_time and combo_peak and avg_speed:
        ...
        if was_found == False:
            # Include run_time, combo_peak, and avg_speed if available
            if leaderboard_file_name == "leaderboard.txt" and run_time and combo_peak and avg_speed:
                entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak},{avg_speed}\n"
    ...
    with open(os.path.join(combined_server_path_rel,score_type), 'r', encoding='utf-8', errors='ignore') as score_file:
        ...
        for line in score_file:
            ...
            if score_type == "leaderboard.txt":
                try:
                    ...
                    try:
                        ...
                        # Fall back to old format without time/combo/avg
                        try:
            ...
            if name_allowed_to_sort:
                scores.append([car, name, score, input_method, run_time, combo_peak, avg_speed])
    ...
    for i,score in enumerate(scores):
        ...
        for classcore in score:
            ...
            if scorecounter <= scorelength:
                ...
                # Get run_time, combo_peak, and avg_speed if available (indices 4, 5, 6)
                ...
                # Format entry with optional time, combo, and avg speed
                extra_info = ""
                if run_time and combo_peak and avg_speed:
