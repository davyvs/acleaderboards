...
def score_find(selected_log: str, previous_log: str, combined_server_path_rel: str):
    ...
    for index_log_line,log_line in enumerate(selected_log_lines):
        ...
        or (shmoovin_match := re.findall(".* \[INF\] CHAT: (.*) \(\d*\): just scored a (\d*)", log_line))):
            ...
            # Try to extract time, combo, avg speed, and tandem status from the same line
            run_time = None
            combo_peak = None
            avg_speed = None
            tandem_status = None
            
            # Match pattern for OVERTAKE with avg speed: "just scored a 12345 | Time: 00:00.00 | Combo: 123.4x | Avg: 150 km/h"
            if time_combo_match := re.findall(r"just scored a \d* \| Time: ([\d:\.]+) \| Combo: ([\d\.]+)x \| Avg: ([\d\.]+) km/h", log_line):
                run_time = str(time_combo_match[0][0])
                combo_peak = str(time_combo_match[0][1])
                avg_speed = str(time_combo_match[0][2])
                logging.debug(f"found time, combo, and avg speed: Time={run_time}, Combo={combo_peak}, Avg={avg_speed}")
            
            # Match pattern for DRIFT with tandem: "just scored a 12345 | Time: 00:00.00 | Combo: 1.5x | Tandem: [▣]"
            elif time_combo_tandem_match := re.findall(r"just scored a \d* \| Time: ([\d:\.]+) \| Combo: ([\d\.]+)x \| Tandem: (\[.\])", log_line):
                run_time = str(time_combo_tandem_match[0][0])
                combo_peak = str(time_combo_tandem_match[0][1])
                tandem_status = str(time_combo_tandem_match[0][2])  # Will be "[▣]" or "[ ]"
                avg_speed = None
                logging.debug(f"found drift score with tandem: Time={run_time}, Combo={combo_peak}, Tandem={tandem_status}")
            
            # Match pattern for DRIFT without tandem (old format): "just scored a 12345 | Time: 00:00.00 | Combo: 1.5x"
            elif time_combo_match := re.findall(r"just scored a \d* \| Time: ([\d:\.]+) \| Combo: ([\d\.]+)x", log_line):
                run_time = str(time_combo_match[0][0])
                combo_peak = str(time_combo_match[0][1])
                avg_speed = None
                tandem_status = None
                logging.debug(f"found time and combo (drift): Time={run_time}, Combo={combo_peak}")
            
            score_find_additional(name, score, leaderboard_file_name, index_log_line, selected_log, previous_log, combined_server_path_rel, run_time, combo_peak, avg_speed, tandem_status)
...
def score_find_additional(name: str,score: float, leaderboard_file_name: str,index_log_line: int, selected_log: str, previous_log: str, combined_server_path_rel: str, run_time: str = None, combo_peak: str = None, avg_speed: str = None, tandem_status: str = None):
    ...
    if name_allowed:
        ...
        write_score(name, score, car, input_method, leaderboard_file_name, combined_server_path_rel, run_time, combo_peak, avg_speed, tandem_status)
...
def input_find(index_log_line: int ,name: str, selected_log: str, previous_log: str) -> str:
    ...
    if input_method == "Unknown":
        ...
        try:
            ...
            with open(str(previous_log), encoding='utf-8', errors='ignore') as second_log_file:
...
def find_car(index_log_line: int ,name: str, selected_log: str, previous_log: str) -> str:
    ...
    if car == "unknown":
        ...
        with open(str(previous_log), encoding='utf-8', errors='ignore') as f:
...
def write_score(name, score, car, input_method, leaderboard_file_name, combined_server_path_rel, run_time=None, combo_peak=None, avg_speed=None, tandem_status=None):
    ...
    with open(f"{os.path.join(combined_server_path_rel,leaderboard_file_name)}", encoding='utf-8', errors='ignore', mode="r+") as score_file:
        ...
        for score_file_line in score_file_lines:
            ...
            if name in score_file_line and car in score_file_line:
                    ...
                    if leaderboard_file_name == "leaderboard.txt" and float(score) > float(old_score):
                        # Include run_time, combo_peak, avg_speed, and tandem_status if available
                        if run_time and combo_peak and avg_speed and tandem_status:
                            entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak},{avg_speed},{tandem_status}\n"
                        elif run_time and combo_peak and tandem_status:
                            entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak},{tandem_status}\n"
                        elif run_time and combo_peak and avg_speed:
                            entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak},{avg_speed}\n"
                        elif run_time and combo_peak:
                            entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak}\n"
                        else:
        ...
        if was_found == False:
            # Include run_time, combo_peak, avg_speed, and tandem_status if available
            if leaderboard_file_name == "leaderboard.txt" and run_time and combo_peak and avg_speed and tandem_status:
                entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak},{avg_speed},{tandem_status}\n"
            elif leaderboard_file_name == "leaderboard.txt" and run_time and combo_peak and tandem_status:
                entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak},{tandem_status}\n"
            elif leaderboard_file_name == "leaderboard.txt" and run_time and combo_peak and avg_speed:
                entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak},{avg_speed}\n"
            elif leaderboard_file_name == "leaderboard.txt" and run_time and combo_peak:
                entry = f"{car},{name},{score},{input_method},{run_time},{combo_peak}\n"
            else:
                entry = f"{car},{name},{score},{input_method}\n"
            score_file_lines_new.append(entry)
    ...
    with open(os.path.join(combined_server_path_rel,score_type), 'r', encoding='utf-8', errors='ignore') as score_file:
        ...
        for line in score_file:
            ...
            if score_type == "leaderboard.txt":
                try:
                    # Try to parse with time, combo, avg speed, and tandem
                    car, name, score, input_method, run_time, combo_peak, avg_speed, tandem_status = line.split(',')
                    combo_peak = combo_peak.strip()
                    avg_speed = avg_speed.strip()
                    tandem_status = tandem_status.strip()
                
                except ValueError:
                    # Try to parse with time, combo, and tandem (no avg speed - DRIFT)
                    try:
                        car, name, score, input_method, run_time, combo_peak, tandem_status = line.split(',')
                        combo_peak = combo_peak.strip()
                        tandem_status = tandem_status.strip()
                        avg_speed = None
                    
                    except ValueError:
                        # Try to parse with time, combo, and avg speed (no tandem - OVERTAKE)
                        try:
                            car, name, score, input_method, run_time, combo_peak, avg_speed = line.split(',')
                            combo_peak = combo_peak.strip()
                            avg_speed = avg_speed.strip()
                            tandem_status = None
                        
                        except ValueError:
                            # Try to parse with time and combo but no avg speed or tandem
                            try:
                                car, name, score, input_method, run_time, combo_peak = line.split(',')
                                combo_peak = combo_peak.strip()
                                avg_speed = None
                                tandem_status = None
                            
                            except ValueError:
                                # Fall back to old format without time/combo/avg/tandem
                                try:
            ...
            if name_allowed_to_sort:
                scores.append([car, name, score, input_method, run_time, combo_peak, avg_speed, tandem_status])
    ...
    for i,score in enumerate(scores):
        ...
        for classcore in score:
            ...
            if scorecounter <= scorelength:
                ...
                # Get run_time, combo_peak, avg_speed, and tandem_status if available (indices 4, 5, 6, 7)
                run_time = classcore[4] if len(classcore) > 4 and classcore[4] else None
                combo_peak = classcore[5] if len(classcore) > 5 and classcore[5] else None
                avg_speed = classcore[6] if len(classcore) > 6 and classcore[6] else None
                tandem_status = classcore[7] if len(classcore) > 7 and classcore[7] else None
                
                # Format entry with optional time, combo, avg speed, and tandem
                extra_info = ""
                if run_time and combo_peak and avg_speed and tandem_status:
                    extra_info = f" | {run_time} | {combo_peak}x | {avg_speed} km/h | {tandem_status}"
                elif run_time and combo_peak and tandem_status:
                    extra_info = f" | {run_time} | {combo_peak}x | {tandem_status}"
                elif run_time and combo_peak and avg_speed:
                    extra_info = f" | {run_time} | {combo_peak}x | {avg_speed} km/h"
                elif run_time and combo_peak:
...
def format_webhook(combined_server_path_rel: str, main_loop_counter: int, shmoovin_score : str, lap_times: str, sector_times: list, show_times: bool, show_shmoovin: bool, show_sectors: bool, shmoovin_type: str) -> int:
    ...
    if list_of_fields:
        for i, entry in enumerate(list_of_fields):
            send_message_counter = format_webhook_second_stage(entry, field_basics, send_message_counter, i, combined_server_path_rel, name, color)     
    else:
        i = 0
        send_message_counter = format_webhook_second_stage(list_of_fields, field_basics, send_message_counter, i, combined_server_path_rel, name, color)     

    return(send_message_counter)

def format_webhook_second_stage(entry, field_basics, send_message_counter, i, combined_server_path_rel, name, color):
